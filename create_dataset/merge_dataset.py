...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        help='input dir 2',
        default='/media/kosuke/SANDISK/meshdata/Hanging-ObjectNet3D-DoubleFaces/cup_key')
    ...
    cmd = 'sudo cp -r ' + args.input_dir_1 + '/clip_info ' + args.out_dir
    print(cmd)
    subprocess.call(
        [cmd],
        shell=True)
    cmd = 'sudo cp -r ' + args.input_dir_1 + '/debug_axis ' + args.out_dir
    print(cmd)
    subprocess.call(
        [cmd],
        shell=True)
    cmd = 'sudo cp -r ' + args.input_dir_1 + '/depth ' + args.out_dir
    print(cmd)
    subprocess.call(
        [cmd],
        shell=True)
    cmd = 'sudo cp -r ' + args.input_dir_1 + \
        '/hanging_points_depth ' + args.out_dir
    print(cmd)
    subprocess.call(
        [cmd],
        shell=True)
    cmd = 'sudo cp -r ' + args.input_dir_1 + '/heatmap ' + args.out_dir
    print(cmd)
    subprocess.call(
        [cmd],
        shell=True)
    cmd = 'sudo cp -r ' + args.input_dir_1 + '/rotations ' + args.out_dir
    print(cmd)
    subprocess.call(
        [cmd],
        shell=True)
    cmd = 'sudo cp -r ' + args.input_dir_1 + '/intrinsics ' + args.out_dir
    print(cmd)
    subprocess.call(
        [cmd],
        shell=True)
    ...
    for idx in range(files_2_last_idx):
        ...
        subprocess.call(
            [cmd],
            shell=True)
        cmd = 'sudo cp ' + args.input_dir_1 + '/hanging_points_depth/{:05}.npy '.format(idx) + osp.join(
            args.out_dir, 'hanging_points_depth', '{:05}.npy'.format(files_1_last_idx + idx + 1))
        print(cmd)
        subprocess.call(
            [cmd],
            shell=True)
