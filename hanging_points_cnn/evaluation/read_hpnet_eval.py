...
parser.add_argument(
    ...
    default='/media/kosuke55/SANDISK-2/meshdata/ycb_sim_eval_hanging/eval_gan/diff') # sim hanging
...
parser.add_argument(
    ...
    default='/media/kosuke55/SANDISK-2/meshdata/ycb_sim_eval_hanging/eval_shapenet/diff') # sim hanging
...
for path in paths:
    ...
    for label in labels:
        ...
        for pos_diff, distance, angle in zip(
                data[str(label)]['pos_diff'],
                data[str(label)]['distance'],
                data[str(label)]['angle']):
            result[category]['pos_diff'].append(pos_diff)
            result[category]['distance'].append(distance)
            result[category]['angle'].append(angle)
            total['pos_diff'].append(pos_diff)
            total['distance'].append(distance)
...
for key in result.keys():
    ...
    result[key]['distance_mean'] = np.mean(np.abs(result[key]['distance'])) * 1000
...
for path in paths:
    ...
    for label in labels:
        ...
        for pos_diff, distance, angle in zip(
                s_data[str(label)]['pos_diff'],
                s_data[str(label)]['distance'],
                s_data[str(label)]['angle']):
            shapenet_result[category]['pos_diff'].append(pos_diff)
            shapenet_result[category]['distance'].append(distance)
            shapenet_result[category]['angle'].append(angle)
            s_total['pos_diff'].append(pos_diff)
            s_total['distance'].append(distance)
...
for key in shapenet_result.keys():
    ...
    shapenet_result[key]['distance_mean'] = np.mean(np.abs(shapenet_result[key]['distance'])) * 1000
    shapenet_result[key]['angle_mean'] = float(np.mean(shapenet_result[key]['angle'])) / np.pi * 180

if print_mode == 0:
    print('\nprint mode 0: TP FP FN dx dy dz angle')
    ...
    print('\nprint mode 1: TP FP FN dx dy dz dl angle')
    for key in sorted(result.keys()):
        data = result[key]
        s_data = shapenet_result[key]
        ...
        if s_data['angle'] == [] and data['angle'] == []:
            ...
            print('{}  &{} &{} &{} &- &- &- &- &- &{} &{} &{} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{:.2f} \\\\'.format(
                ...
                len(data['tp']),
                len(data['fp']), len(data['fn']),
                data['pos_mean'][0], data['pos_mean'][1], data['pos_mean'][2],
                data['distance_mean'],
                data['angle_mean']))
        elif s_data['angle'] != [] and data['angle'] == []:
            print('{} &{} &{} &{} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{} &{} &{} &- &- &- &- &-\\\\'.format(
            ...
            print('{}  &{} &{} &{} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{} &{} &{} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{:.2f} \\\\'.format(
                ...
                s_data['distance_mean'],
                s_data['angle_mean'],
                len(data['tp']), len(data['fp']), len(data['fn']),
                data['pos_mean'][0], data['pos_mean'][1], data['pos_mean'][2],
                data['distance_mean'],
                data['angle_mean']))

    total['pos_mean'] = (np.mean(np.abs(total['pos_diff']), axis=0) * 1000).tolist()
    total['distance_mean'] = np.mean(total['distance']) * 1000
    total['angle_mean'] = float(np.mean(total['angle'])) / np.pi * 180

    s_total['pos_mean'] = (np.mean(np.abs(s_total['pos_diff']), axis=0) * 1000).tolist()
    s_total['distance_mean'] = np.mean(s_total['distance']) * 1000
    s_total['angle_mean'] = float(np.mean(s_total['angle'])) / np.pi * 180

    print('{}  &{} &{} &{} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{} &{} &{} &{:.2f} &{:.2f} &{:.2f} &{:.2f} &{:.2f} \\\\'.format(
        'total',
        s_total['tp'], s_total['fp'], s_total['fn'],
        s_total['pos_mean'][0], s_total['pos_mean'][1], s_total['pos_mean'][2],
        s_total['distance_mean'],
        s_total['angle_mean'],
        total['tp'], total['fp'], total['fn'],
        total['pos_mean'][0], total['pos_mean'][1], total['pos_mean'][2],
        total['distance_mean'],
        total['angle_mean']))

elif print_mode == 2:
    print('\nprint mode 2: TP FP FN dl angle')
    for key in sorted(result.keys()):
        data = result[key]
        s_data = shapenet_result[key]
